...
aws s3 sync ./build/templates/ s3://$CORNCHAT_PRIVATE_BUCKET/cloudformation/
...
if echo $STATUS | grep -q 'does not exist'; then
  ...
  aws cloudformation create-stack \
    ...
    --template-url https://s3.amazonaws.com/$CORNCHAT_PRIVATE_BUCKET/cloudformation/template.yaml \
